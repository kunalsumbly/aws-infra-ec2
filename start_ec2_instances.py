...
def lambda_handler(event, context):
    """
    Lambda function to start EC2 instances at scheduled times.
    ...
    try:
        ...
        if current_date > end_date:
            print(f"Current date {current_date.strftime('%Y-%m-%d')} is past end date {end_date_str}. Skipping instance start.")
            return {
                'statusCode': 200,
    ...
    try:
        ...
        for reservation in response['Reservations']:
            for instance in reservation['Instances']:
                ...
                if state == 'stopped':
                    ...
                    print(f"Instance {instance_id} is stopped and will be started")
                elif state == 'running':
                    print(f"Instance {instance_id} is already running - skipping")
                else:
        ...
        if instances_to_start:
            ...
            print(f"Start request sent for instances: {instances_to_start}")
            
            # Log the starting instances
            starting_instances = []
            ...
            return {
                'statusCode': 200,
                'body': json.dumps({
                    'message': f'Successfully initiated start for {len(instances_to_start)} instances',
                    'started_instances': starting_instances,
        ...
        return {
            'statusCode': 500,
            'body': json.dumps({
                'error': f'Failed to start instances: {str(e)}',
                'execution_time': datetime.now(timezone.utc).isoformat()
            })
        }